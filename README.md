...
### 🌟 v0.6.7 Release Highlights (2026-09-24, measurements and wording only — drafter v1.0 and the launcher pin are unchanged)
* 📏 **No-speculation baseline measured for the first time**: the same target, image, flags and mounts with every `--speculative-*` flag removed decodes the 50-prompt battery at a flat **10.30 tok/s C1 / 39.56 tok/s C4** aggregate (thinking off) — the weight-streaming bound of a 24.85 GB target on 273 GB/s. Against the production configuration (drafter v1.0, K = 12, 64K head, FP8 draft KV: 35.66 / 108.83 tok/s in the paired-sweep cell) that is **3.46× at C1 and 2.75× at C4**; per domain at C1: code 5.6×, math 6.0×, tool calls 8.5×, prose 2.1×, instruction-following 2.1× (§1). The drafter also costs KV capacity: without it the pool hits the 4 × 262,144-token cap with 21 GB still unallocated.
* 🎯 **"Lossless" now means exactly what we measured**: 40 greedy 256-token continuations captured on the speculative server and on the no-speculation twin differ on 28/40 prompts, every first divergence at a top-2 logprob margin ≤ 0.25 (a bf16 near-tie); two runs of the speculative server itself differ on 6/40 with the same margins. Speculative decoding changes the floating-point path, not the target's choice where the target is decided — lossless up to floating-point numerics, not bit-identical. `bench/compose_nospec.py` and `bench/lossless_probe.py` reproduce both measurements.
* ⚠️ **Reference-loop caveat for anyone evaluating drafters on this hybrid offline**: transformers' `Cache.crop` does not roll back the Gated-DeltaNet recurrent state, so the HF/z-lab reference DFlash loop keeps rejected drafts in the target's memory and is not lossless on Qwen3.5-class targets (it inflated measured acceptance 1.9× in our 100-prompt diagnostic before we fixed our copy). Reported upstream as huggingface/transformers#49036 and z-lab/dflash#172. SGLang's verify path is unaffected — that is what the measurement above shows.
...
**No speculation vs the production configuration (2026-09-23).** A throwaway twin of the production container was composed from its own `docker inspect` with every `--speculative-*` flag removed (`bench/compose_nospec.py`) — same target, image, flags, mounts and clock, single tenant — and run through the same 50-prompt battery (thinking off, `temperature 0`, same protocol as above, 1 run):

| Configuration | C1 aggregate | C4 aggregate | C1 medians: code · math · tool calls · prose · IFEval |
|---|:---:|:---:|---|
| Kearuga target, **no drafter** | **10.30 tok/s** | **39.56 tok/s** | 10.4 · 10.4 · 12.7 · 10.4 · 10.5 |
| Kearuga target + drafter v1.0 (K = 12, 64K head, FP8 draft KV — the production configuration; paired-sweep cell, 2026-09-21) | 35.66 tok/s (**3.46×**) | 108.83 tok/s (**2.75×**) | 58.2 · 62.4 · 107.6 · 22.0 · 22.0 (5.6× · 6.0× · 8.5× · 2.1× · 2.1×) |

Without a drafter every domain decodes at the same ≈ 10.3 tok/s: the loop is weight-streaming-bound (24.85 GB per token over 273 GB/s ≈ 11 tok/s) and C4 scales almost linearly (3.84×). The KV pool without the drafter hits the `4 × 262,144 = 1,048,576`-token cap with 21.0 GB still unallocated, versus 932,355 tokens with it. *Method note:* the production container's own in-window run was discarded because the live service had 2–4 concurrent client requests during it (visible in the server log); the speculative row is the nearest single-tenant measurement of the identical configuration on the same bench line.

**Losslessness, measured (2026-09-23).** `bench/lossless_probe.py` captured the 40 Fidelity prompts greedily (256 tokens, thinking off, top-2 logprobs) three times: on the speculative server (A), on the no-speculation twin (OFF), and on the speculative server again (B). A vs OFF: 28/40 continuations diverge, every first divergence at a top-1 − top-2 logprob margin of 0.0 / 0.125 / 0.25. A vs B: 6/40, same margins. No divergence occurs where the target is confident — speculative verification reproduces the target's argmax wherever the argmax is decided, and differs only where two candidates sit within bf16 rounding of each other, which two runs of the *same* server also do. That is the precise sense of "lossless" used in this repository.
